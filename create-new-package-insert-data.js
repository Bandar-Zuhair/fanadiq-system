    ...
    if (clickedButtonId === "clint_inputs_submit_icon") {
        ...
        if (document.getElementById("downloaded_pdf_hotel_data_page").style.display === "none") {
            ...
            allHotelRows.forEach((hotelRow, index) => {
                ...
                if ((validCities.includes(currentCity) && lastCity && lastCity !== currentCity && validCities.includes(lastCity)) || (lastCity === "بونشاك" && currentCity === "بالي") || (lastCity === "بالي" && currentCity === "بونشاك")) {
                    ...
                    let flightRowTableDivContent = `
                        ...
                        <div><p id='flight_adult_person_amount_${insertedFlightDataDivUniqueId}'>${document.getElementById("adult_package_person_amount_input_id").value}</p>${
                        document.getElementById("infant_package_person_amount_input_id").value
                            ? `<p id="flight_infant_person_amount_${insertedFlightDataDivUniqueId}">${document.getElementById("infant_package_person_amount_input_id").value}</p>`
                            : ""
                    }</div>
        ...
        if (
            hotelNameReadyText === "" ||
            hotelCheckInReadyText === "" ||
            hotelCheckOutReadyText === "" ||
            hotelRoomTypeDescriptionInput === "" ||
            hotelUnitAmountInput === "" ||
            ...
            /* Set the 'insertedHotelDataDivUniqueId' value based on the following condition */
            let insertedHotelDataDivUniqueId = document.getElementById("store_google_sheet_hotel_uniuqe_id_name_value").innerText !== "" ? document.getElementById("store_google_sheet_hotel_uniuqe_id_name_value").innerText : 1;
            ...
            let hotelRowTableDivContent = `
                ...
                <div>
                    ...
                    ${
                        hotelUnitAmountInput_2 && document.getElementById("hotel_second_room_data_input_div").style.display !== "none"
                            ? `<p style="width: 100%; background: rgb(40, 62, 68); color: white">+ </p><p id="hotel_total_unit_2_${insertedHotelDataDivUniqueId}" style="width: 100%; background: rgb(40, 62, 68); color: white">${storeHotelTotalUnitNumber_2}</p>`
                            : ""
                    }
                </div>
                <div>
            ...
            function handleClickEvent(element) {
                ...
                element.addEventListener("mousedown", (event) => {
                    if (!isTouchEvent) {
                        // Only execute if it is not a touch event
...
downloadPdfWithCustomName = async function (pdfName) {
    ...
    let page1Divs = ["downloaded_pdf_clint_data_page", "downloaded_pdf_flight_data_page", "downloaded_pdf_hotel_data_page", "downloaded_pdf_package_including_data_page"];

    let page2Divs = [
        "downloaded_pdf_clint_movements_data_page", // Add only to page 2
    ];

    let page3Divs = ["downloaded_pdf_important_notes_data_page"];
